  ...
  methods: {
    async updateView() {
      ...
      for (let a = 0; a < e.length; a++) {
        let d = e[a],
          i = "default" === d.data.itemId ? a : d.data.itemId;
    ...
    async onTapCollapseItem(t) {
      "accordion" === this.data.type && await this.foldAllExpandItem(t), this.setCollapseItemStatus(t, !t.data.isExpandContent), t.data.isExpandContent ? this.triggerEvent("linfold", {
    ...
    async setCollapseItemStatus(t, e) {
      if (e) t.expandContent(), this.data._expandItems.push(t);
      else {
        await t.foldContent();
